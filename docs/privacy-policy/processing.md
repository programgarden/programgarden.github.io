...
### ProgramGarden AI 데이터 처리

ProgramGarden AI 서비스 제공을 위해 이용자의 질의(채팅 메시지)가 처리됩니다. **ProgramGarden AI는 이용자의 대화 내용을 자체 AI 모델의 학습 데이터로 절대 사용하지 않습니다.**

*   처리되는 데이터는 질의 텍스트, 대화 컨텍스트 등이며, 이메일·이름 등 개인 식별 정보는 AI 처리에 사용되지 않습니다.
*   ProgramGarden AI는 서비스 제공을 위해 제3자 AI 기업의 API를 활용하며, 회사는 이용자 데이터를 모델 학습에 사용하지 않는 기업을 우선적으로 선정합니다. 다만, 제3자 AI 기업의 데이터 처리 정책은 해당 기업의 약관에 따릅니다.
*   이용자는 AI 서비스 이용 시 민감한 개인정보(주민등록번호, 금융 비밀번호 등)를 입력하지 않을 것을 권장합니다.
...
- **기술적 조치:**
    ...
    - **채팅 데이터 보호:** AI 채팅 이력은 보안 데이터베이스에 저장되며, 전송 구간 TLS 암호화 및 저장 시 클라우드 기본 암호화 적용
    - 보안프로그램 설치 및 갱신
- **물리적 조치:** 클라우드 데이터 센터의 물리적 접근통제
...
### 1. 주요 데이터 저장 위치
이용자의 개인정보 및 서비스 데이터는 원칙적으로 **국내 데이터 센터(서울 리전)**에 저장되며, 대한민국 내에서 처리됩니다.

### 2. ProgramGarden AI 데이터 처리

ProgramGarden AI 서비스 제공 과정에서 이용자의 질의 텍스트 및 대화 컨텍스트가 처리되며, 제3자 AI 기업의 API를 통해 전송될 수 있습니다. 이메일·이름 등 개인 식별 정보는 AI 처리에 사용되지 않습니다.

### 3. 보호 조치
ProgramGarden AI의 데이터 처리 시 다음 보호 조치를 적용합니다:
- 전송 구간 TLS 암호화
- 개인 식별 정보 최소화 (질의 텍스트만 처리, 이메일/이름 등 미사용)
- 이용자 데이터를 모델 학습에 사용하지 않는 제3자 AI 기업 우선 선정
...
### 2. AI 자동 태깅
공유된 프로젝트는 ProgramGarden AI를 활용하여 자동으로 카테고리 태깅이 수행됩니다. 이 과정에서 프로젝트의 메타데이터(코드 구조, 설명 등)가 처리됩니다.

### 3. 벡터 임베딩
공유된 프로젝트의 검색 기능 지원을 위해 ProgramGarden AI를 통한 벡터 임베딩이 생성되어 저장됩니다.
